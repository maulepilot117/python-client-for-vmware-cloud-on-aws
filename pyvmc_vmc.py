...
def vtc_operations_json(strProdURL, org_id, session_token, json_body):
...
def get_deployments_json(strProdURL,org_id, session_token):
    ...
    myHeader = {'csp-auth-token': session_token}
    myURL = f"{strProdURL}/api/inventory/{org_id}/core/deployments"
    response = requests.get(myURL, headers=myHeader)
    json_response = response.json()
    if response.status_code == requests.status_codes.ok:
        return json_response
...
def get_group_id_json(strProdURL, group, org_id, session_token):
    myHeader = {'csp-auth-token': session_token}
    myURL = f"{strProdURL}/api/inventory/{org_id}/core/deployment-groups"
    response = requests.get(myURL, headers=myHeader)
    json_response = response.json()
    if response.status_code == requests.status_codes.ok:
        return json_response
...
def get_resource_id_json(strProdURL, org_id, sddc_group_id, session_token):
    myHeader = {'csp-auth-token': session_token}
    myURL = f"{strProdURL}/api/network/{org_id}/core/network-connectivity-configs/?group_id={sddc_group_id}"
    response = requests.get(myURL, headers=myHeader)
    json_response = response.json()
    if response.status_code == requests.status_code.ok:
        return json_response
    else:
        vmc_error_handling(response)
        sys.exit(1)
...
#
def create_sddc_group_json(strProdURL, org_id, session_token, body):
...
def ext_get_group_info_json(strProdURL, org_id, resource_id, session_token):
    myHeader = {'csp-auth-token': session_token}
    myURL = f"{strProdURL}/api/network/{org_id}/core/network-connectivity-configs/{resource_id}/?trait=AwsVpcAttachmentsTrait,AwsRealizedSddcConnectivityTrait,AwsDirectConnectGatewayAssociationsTrait,AwsNetworkConnectivityTrait"
    response = requests.get(myURL, headers=myHeader)
    json_response = response.json()
    if response.status_code == requests.status_codes.ok:
        return json_response
...
def get_route_tables_json(strProdURL, resource_id, org_id, session_token):
    myHeader = {'csp-auth-token': session_token}
    myURL = f"{strProdURL}/api/network/{org_id}/core/network-connectivity-configs/{resource_id}/route-tables"
    response = requests.get(myURL, headers=myHeader)
    json_response = response.json()
    if response.status_code == requests.status_codes.ok:
        return json_response
...
def vtgw_route_json(strProdURL, org_id, resource_id, mem_ext_id,session_token):
    myHeader = {'csp-auth-token': session_token}
    myURL = f"{strProdURL}/api/network/{org_id}/core/network-connectivity-configs/{resource_id}/route-tables/{mem_ext_id}/routes"
    response = requests.get(myURL, headers=myHeader)
    json_response = response.json()
    if response.status_code == requests.status_codes.ok:
        return json_response
